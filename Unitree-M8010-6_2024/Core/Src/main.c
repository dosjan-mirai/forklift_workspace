...
/* USER CODE BEGIN PV */
MOTOR_send Motor1_send;   //以全局变量声明电机控制结构体和电机数据结构体，方便在故障时通过debug查看变量值
MOTOR_send cmd1;
MOTOR_recv Motor1_recv;
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1)
	{
		while(i!=10)
		{
		SERVO_Send_recv(&Motor1_send, &Motor1_recv);
		HAL_Delay(100);
		i++;
		}
//		cmd.T=0;
